...
$(function(){
    ...
    $(".add-field").click(function() {
        ...
        $(".alert_from").remove();
        var myArray = {type: $(this).data('type'), type_name: $(this).data('typename'), is_required: $(this).data('is_required'), is_options: $(this).data('is_options'), type_common: $(this).data('type_common'), input_id: 0,is_default_value: $(this).data('default_value'),default_value: ''};
    ...
    $('.save_dy_form').click(function(){  
        ...
        $('.field').each(function() {
            ...
            //field label
            var defaultVal = $this.find('.default-val').val();
                
            //field label
            var input_id = $this.attr('data-input_id'); 
...
function addField(myArray) {
    ...
    var returnformITEM = '';
    var default_section=myArray.is_default_value;
    var default_value=myArray.default_value;
    //multile file upload option

    var default_input ='<input type="text" class="form-control default-val"  value="'+default_value+'" placeholder="Default value" data-input_id="'+input_id+'" >';
    if(fieldType == 'textarea')
    { 
    default_input ='<textarea type="text" class="form-control default-val"   placeholder="Default value" data-input_id="'+input_id+'">'+default_value+'</textarea>';    
    }
    if(fieldType == 'file')
    {
        includeChoicesHTML = '' +
   ...
   /* return '' +
        ...
        returnformITEM +='<div class="row form_row field" data-type="' + fieldType + '" data-input_id="'+input_id+'">';
        ...
        {
           returnformITEM +='<input type="text" class="form-control field-label"  value="" placeholder="Field Name" data-input_id="'+input_id+'">';
        }
        returnformITEM +='</div>';
        ...
        {
        returnformITEM +='<div class="col-xs-12">&nbsp;</div>';
        returnformITEM +='<label for="" class="col-sm-2 control-label">Default value: </label>';
        returnformITEM +='<div class="col-sm-5">'+default_input;
    ...
    $.getJSON(loadformurl+'?form_id=' + formID, function(data) {
        ...
        $.each(data.form_privilages, function(i, items)
        {
            if(items.privilege_key == 'add'){
                $.each(items.privilege_department_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#add_departmentid option[value='+item+']').length > 0)
                        {
                ...
                $.each(items.privilege_user_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#add_userid option[value='+item+']').length > 0)
                        {
                ...
                $.each(items.privilege_department_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#edit_departmentid option[value='+item+']').length > 0)
                        {
                ...
                $.each(items.privilege_user_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#edit_userid option[value='+item+']').length > 0)
                        {
                ...
                $.each(items.privilege_department_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#delete_departmentid option[value='+item+']').length > 0)
                        {
                ...
                $.each(items.privilege_user_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#delete_userid option[value='+item+']').length > 0)
                        {
                ...
                $.each(items.privilege_department_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#view_departmentid option[value='+item+']').length > 0)
                        {
                ...
                $.each(items.privilege_user_array, function(i, item)
                {
                    if(item != '')
                    {
                        if($('#view_userid option[value='+item+']').length > 0)
                        {
        ...
        if (data.inputs) {
            ...
            $.each( data.inputs, function( k, v ) {
                //Add the field
                var myArray = {type: v['type'], type_name: v['input_type_name'], is_required: v['is_required'], is_options: v['is_options'], type_common: v['type_common'], input_id: v['input_id'],is_default_value: v['is_default_value'],default_value: v['default_value']};
                $(addField(myArray)).appendTo('#form-fields').hide().slideDown('fast');
