...
function generateFormEdit(formID,url,action,unique,del_url,attch_url) 
{
    ...
    $.getJSON(url+'?form_id=' + formID+'&action='+action+'&uq_id='+unique, function(data) 
    {
		...
		if(($.inArray(user_id,edit_permissions) != -1) || ($.inArray(user_id,view_permissions) != -1) || (user_role == 1))
		{
    		...
    		//add fields
    		$('#sjfb-fields').append(addFieldHTML(fieldType,inputId,typeId,val,files,res_id,read_only));
            ...
            if (v['choices']) {
                ...
                $.each( v['choices'], function( k, v ) {
                    ...
                    if (fieldType == 'select') {
                        ...
                        var choiceHTML = '<div class="radio"><label><input type="radio" name="' + uniqueID + '[]"' + selected + ' value="' + v['label'] + '" '+read_only_text+'>' + v['label'] + '</label></div><input type="hidden" name="resp_id[]" value="'+res_id+'">';
                        $currentField.find(".choices").append(choiceHTML);
                    }

                    else if (fieldType == 'checkbox') {
                        var selected = (v['sel'] == 1) ? ' checked' : '';
                        var choiceHTML = '<div class="checkbox"><label><input type="checkbox" name="' + uniqueID + '[]"' + selected + ' value="' + v['label'] + '" '+read_only_text+'>' + v['label'] + '</label></div><input type="hidden" name="resp_id[]" value="'+res_id+'">';
                        $currentField.find(".choices").append(choiceHTML);
                    }
        ...
        function addFieldHTML(fieldType,uniqueID,typeId,val,files,res_id,read_only) 
        {
