...
class Common {
   ...
   public function get_modules()
    ...
    public function audit_clear_notification_check(){
        ...
        if($audits_delete_approved_status->count()){
    ...
    public function send_mail($subject)
    {
        if (Auth::user()) 
        {
            try 
            {
                ...
                if($file)
                {
                    $subject['attachments'] =  (isset($subject['attachments']))?$subject['attachments']:array();
                    Mail::send('email', ['request' => $subject], function ($m) use ($datas) {
                            $m->from(config('mail.username'));
                            $m->to($datas['email_to']);
                            $m->subject($datas['subject']);
                            $m->attach($datas['file']);
                            foreach ($datas['attachments'] as $key => $value) 
                            {
                                $m->attach($value['file'],['as' => $value['name']]);
                            }
                        }); 
