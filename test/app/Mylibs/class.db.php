...
namespace vakata\database
{
	...
	{
		public $type		= null;
		public $username	= "config()->get('database.connections.mysql.username')";
		public $password	= '';
		public $database	= "config()->get('database.connections.mysql.database')";
		public $servername	= "config()->get('database.connections.mysql.host')";
	...
	class mysqli_driver extends ADriver
	{
		...
		public function connect() {
			$paswd = config()->get('database.connections.mysql.password');
			$this->lnk = new \mysqli(
	...
	class oracle_driver extends ADriver
	{
		...
		public function connect() {
			$paswd = config()->get('database.connections.mysql.password');
			$this->lnk = ($this->settings->persist) ?
					@oci_pconnect($this->settings->username, $paswd, $this->settings->servername, $this->settings->charset) :
					@oci_connect ($this->settings->username, $paswd, $this->settings->servername, $this->settings->charset);
			if($this->lnk === false) {
