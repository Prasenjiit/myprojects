...
@else
  ...
  <table border="1" class="table table-bordered dataTable" role="grid" id = "workflow_table">
    <tbody>
    <thead>
      <tr>
      <th>{{$language['actions']}}</th>
      <th nowrap>{{$language['workflow name']}} ({{$language['no:docs']}})</th>
      <th>{{$language['stages']}}</th>
      </tr>
    </thead>
...
// Add new stage
  ...
  function addNewStage(id,color,row_no)
  {
    ...
    var counter_stages = $('#hidd_count_stages').val();
    counter_stages++;
