  ...
  var AddGroupTemplate = function(rule_arry) 
   {
    ...
    html+='<button type="button" class="btn btn-xs btn-success add_rule_template" id="add_rule_template_'+rule_count+'" data-add="rule" '+data_attr1+' '+data_attr2+'><i class="fa fa-plus"></i> Add rule </button>';
    if(first != 0)
    {
   ...
   selected = '';
    ...
    html+='<select class="form-control group_stage_case_update" name="builder_group_stage'+rule_count+'_cond" id="builder_group_stage'+rule_count+'_cond" '+data_attr1+' '+data_attr2+'><option value="" '+selected+'>------</option>';
    selected = '';
    if(stage_case == 'if')
    {
      selected='selected="selected"';
   ...
   if(stage_case == 'if')
    ...
    else if(stage_case == 'if_else')
    {
      ...
      html+='<div class="col-xs-4 rule_group_stage_'+rule_count+'" id="rule_group_if_stage_'+rule_count+'" style="'+if_style+'">';
        html+='<label style="">Go to stage</label> <select class="form-control rule_stage_case c_rule_stages_option1_'+stageid+'" id="rule_stages_option1_'+rule_count+'"  name="" '+data_attr1+' '+data_attr2+'>'+temp_stages_option+'</select>';
      html+='</div>';

      html+='<div class="col-xs-8 rule_group_stage_'+rule_count+'" id="rule_group_if_else_stage_'+rule_count+'" style="'+else_style+'">';
        html+='<label style="">Else got to stage</label> <select class="form-control rule_stage_case c_rule_stages_option2_'+stageid+'" id="rule_stages_option2_'+rule_count+'" name="" '+data_attr1+' '+data_attr2+'>'+temp_stages_option+'</select>';
      html+='</div>';
   ...
   $('#builder0').append(temp);*/
    ...
    var addTask = function(task) 
    {
		...
		/////////////// BY USER HELP TEXT ////////////////
		wfTLHTML +='<p class="help_text_user_'+id+'" id="help1_'+id+'" style="font-size:12px; color:#999; display:none;">When this is selected, display the user list.</p>';
		...
		wfTLHTML +='<p class="help_text_user_'+id+'" id="help3_'+id+'" style="font-size:12px; color:#999; display:none;">When this is selected, WF will move to the next stage depending on the sub option selected.</p>';
		...
		wfTLHTML +='<label>Assigned Department</label>';
		wfTLHTML +='<div class="full_widht">';
		...
		/////////////// BY GROUP HELP TEXT ////////////////
		wfTLHTML +='<p class="help_text_group_'+id+'" id="grouphelp1_'+id+'" style="font-size:12px; color:#999; display:none;">When this is selected, display the user list.</p>';
		wfTLHTML +='<p class="help_text_group_'+id+'" id="grouphelp2_'+id+'" style="font-size:12px; color:#999; display:none;">When this is selected, the manager of the user who submitted a form will get a notification automatically.</p>';	
		wfTLHTML +='<p class="help_text_group_'+id+'" id="grouphelp3_'+id+'" style="font-size:12px; color:#999; display:none;">WF will move to the next stage depending on the sub option selected.</p>';
		/////////////// BY GROUP HELP TEXT ////////////////
 
		wfTLHTML +='<div class="form-group" id="bypercentage'+id+'" style="display:none;">';
		wfTLHTML +='<label>Percentage Value</label>';
    ...
    var addAction = function(task) 
   {
      ...
      var action_html ='<div class="panel box">';
          ...
          action_html +='<select name="rule_'+id+'" id="rule_'+id+'" class="form-control rule_change" '+common_data+'>';
      ...
      $.each(rule_data, function(i, item) {
        ...
        if(item.type == 'group')
        {
          ...
          console.log("#add_group_template_"+parenID);
          ...
          /*console.log("First #rule_stages_option1_"+rule_count+" = "+rules_basic.if_stage);*/
          $("#rule_stages_option1_"+rule_count).val(item.if_stage).trigger('change');
           /*console.log("First #rule_stages_option2_"+rule_count+" = "+rules_basic.else_stage);*/
        ...
        else
        {
          ...
          var kll = item.parentid;
            var parenID = myrcArray[kll];
          console.log("#add_rule_template_"+parenID);
      ...
      console.log(edge_data.rules_data)
      return true;
   } 
  ...
  var loadWorkflowtemplate = function(wf) 
      ...
      $.getJSON(loadformurl+'?workflow_id=' + wf, function(data) {
       ...
       var users_options= {sel_count:0,department_users: data.department_users, sel_department_users: sel_department_users}
        ...
        rule_options='<option value="-1" data-column_type="invalid" data-key="invalid" data-object_type="invalid">------</option>';  
              ...
              {
                /*console.log(item);*/
                 rule_options +='<option value="'+item.id+'" data-column_type="'+item.column_type+'" data-key="'+i+'" data-object_type="'+item.object_type+'">'+item.column_name+'</option>';
              });
   ...
   $(document).on("click",".wf_li",function(e) {
       ...
       $("#rwf_div"+liid).show(); /* Show Only Current Right DIV */
        ...
        { 
            ...
            /*console.log('#rule_stages_option1_'+rc+" ="+to_satge)*/
   ...
   var change_stage_group = function(stageid) 
   {
	...
	$(document).on("change keyup paste input",".stage_percentage",function(e) {
	    try {
	        ...
	        //var wf_type =$(this).attr('data-wf_type');
	        /*console.log("percentage stage id "+stageid);
	        console.log("percentage stage percentage "+label);*/
	        nodes.update({
	            id: stageid,
   ...
   $(document).on("click",".preview",function(e) {
    ...
    var from_id =parseInt($(this).attr('data-stage')); 
      ...
      var task= {id: actioncount, dbid: 0, label: label, type: wf_type, from: from_id, to: 0, description:'',with_rule:0,rules_basic:rules_basic_base,rule_data:[]}
        addAction(task);
        actioncount++;
     ...
     var updateRuleCond = function(rule_arry) 
   {
    ...
    var data_attr1 = 'data-rc="'+rc+'" ';
      ...
      /* console.log("filter_option_key = "+filter_option_key);*/
          ...
          value_html +='<select class="form-control rule_filter_update" name="builder_rule_value_'+rc+'" id="builder_rule_value_'+rc+'" '+data_attr1+'" '+data_attr2+'>'; 
              $.each(type_options, function(i, item)
              {
               ...
               /* console.log("item_id"+item.id);
                console.log("item_label"+item.label);*/
        ...
        $(".c_rule_stages_option1_"+stageid).each(function( index )
        {
            console.log("hi2");      
              ...
              if(check_st && c_edgeid == edgeid)
              {
                ...
                console.log("hi3"+to_satge); 
                  if(rule == 1)
                {
                  $("#to_satge"+edgeid).val(to_satge);
                console.log("hi4= edgeid "+edgeid);    
                console.log("hi4= to_satge"+to_satge);    
                edges.update({
                        id: edgeid,
                        to: to_satge
                    });
